...
* 403 if `name` already enrolled to the course.
* 403 if `registrationDate` is 3 days before course `startDate` or after.
* 403 if course is full.
...
* 403 if `cancelDate` is 3 days before course `startDate` or after.
